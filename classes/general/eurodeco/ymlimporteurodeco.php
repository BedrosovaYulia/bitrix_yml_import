...
class CYmlImport
{
    ...
    function ImportYML ($DATA_FILE_NAME, $IBLOCK_ID, $IMPORT_CATEGORY, $ONLY_PRICE, $max_execution_time, $CUR_FILE_POS,$IMPORT_CATEGORY_SECTION, $URL_DATA_FILE2, $ID_SECTION, $CAT_FILTER_I, $price_modifier)
    {
        ...
        if (strlen($strImportErrorMessage) <= 0)
        {
			...
			//Расскомментить для удаления всех свойств ранее созданных модулем
			/*$properties = CIBlockProperty::GetList(Array("sort"=>"asc", "name"=>"asc"), Array("ACTIVE"=>"Y", "IBLOCK_ID"=>$IBLOCK_ID,"CODE"=>'YML%'));
			...
			while ($prop_fields = $properties->GetNext())
			{
			  $arExistingPropsName[]=$prop_fields["NAME"];
			  $arExistingProps[$prop_fields["NAME"]]=$prop_fields;
			}
			
		/*	print "<pre>";
			print_r($arExistingPropsName);
			print "</pre>";*/
			
		/*	print "<pre>";
			print_r($arExistingProps);
			print "</pre>";*/
            ...
            else
            {
                ...
                for ($j = $CUR_FILE_POS; $j < count($xml->shop->offers->offer); $j++)
                {
					...
					//Бедросова 9.09.2014 - вот этот мод по свойствам должен выйти в основной релиз
						...
						$size="";
						$vendor="";
						$vendor=iconv('utf-8',SITE_CHARSET, $xProductNode->vendor);
						$articul=iconv('utf-8',SITE_CHARSET,$xProductNode->vendorCode);
						$strana="";
						$strana=iconv('utf-8',SITE_CHARSET,$xProductNode->country_of_origin);
						
						$country_of_origin=iconv('utf-8',SITE_CHARSET, $xProductNode->country_of_origin);
						$sales_notes=iconv('utf-8',SITE_CHARSET, $xProductNode->sales_notes);
						$manufacturer_warranty=iconv('utf-8',SITE_CHARSET, $xProductNode->manufacturer_warranty);
						$barcode=iconv('utf-8',SITE_CHARSET, $xProductNode->barcode);
						
						//27.03.2015
							
						$arFuturePropsEnum=array();
						...
						foreach($xProductNode->param as $param){
							...
							$atr['name']=iconv('utf-8',SITE_CHARSET,$atr['name']);
							
							if ($atr['name']=='Цвет') $color=iconv('utf-8',SITE_CHARSET,''.$param);
							if ($atr['name']=='Материал') $material=iconv('utf-8',SITE_CHARSET,''.$param);
							...
							//Поищем свойство по названию в инфобооке, в который идет импорт	
							if (in_array($atr['name'].'',$arExistingPropsName)){	
								
								//Будем писать в это свойство
								$ExistingPropID=$arExistingProps[''.$atr['name']]['ID'];
								$ExistingPropType=$arExistingProps[''.$atr['name']]['PROPERTY_TYPE'];
								if ($ExistingPropType=='S'){
									$arFuturePropsEnum[''.$arExistingProps[''.$atr['name']]['CODE']]=iconv('utf-8',SITE_CHARSET,''.$param);
								}
							
							}
							else{
								//Создадим новое строковое свойство и будем писать в него
								
								$NewPropertyCode='YML_'.CUtil::translit($atr['name'], LANGUAGE_ID, array(
										"max_len" => 50,
										"change_case" => 'U', // 'L' - toLower, 'U' - toUpper, false - do not change
										"replace_space" => '_',
										"replace_other" => '_',
								...
								if($NewPropID = $Newibp->Add($arNewPropFields)){
									...
									//И запишем наше новое свойство в массив существующих свойств
									$arExistingPropsName[]=$atr['name'].'';
									$arExistingProps[''.$atr['name']]['ID']=$NewPropID;
									$arExistingProps[''.$atr['name']]['PROPERTY_TYPE']='S';
									$arExistingProps[''.$atr['name']]['CODE']=$NewPropertyCode;
									...
									$arFuturePropsEnum[''.$NewPropertyCode]=iconv('utf-8',SITE_CHARSET,''.$param);
...
$description=$xProductNode->description;
                    ...
                    if ($arr = $res->Fetch())
                    {
                        ...
                        if ($ONLY_PRICE!='Y')
                        {
							...
							CIBlockElement::SetPropertyValueCode($PRODUCT_ID, "VENDOR",$vendor);
							CIBlockElement::SetPropertyValueCode($PRODUCT_ID, "sales_notes",$sales_notes);
							CIBlockElement::SetPropertyValueCode($PRODUCT_ID, "country_of_origin",$country_of_origin);
							CIBlockElement::SetPropertyValueCode($PRODUCT_ID, "CML2_ARTICLE",$articul);
							
							CIBlockElement::SetPropertyValueCode($PRODUCT_ID, "barcode",$barcode);
							CIBlockElement::SetPropertyValueCode($PRODUCT_ID, "manufacturer_warranty",$manufacturer_warranty);
							
						/*	print "<pre>";
							print_r($arFuturePropsEnum);
							print "</pre>";

							die();*/
							
							foreach($arFuturePropsEnum as $key=>$val){
							
								CIBlockElement::SetPropertyValueCode($PRODUCT_ID, $key,$val);
